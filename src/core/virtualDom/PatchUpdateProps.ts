...
export default class PatchUpdateProps extends Patchable {
  ...
  public run(): void {
    ...
    if (isDomNode || isCompNode) {
      ...
      const { attributes, events }: Riact.TPatchUpdatePropsPayload = this
        .patchData.payload as Riact.TPatchUpdatePropsPayload;
      for (const key in attributes as Riact.TObject) {
        if (attributes.hasOwnProperty(key)) {
          ...
          if (isDomNode) {
            if (key === PROP_STYLE) {
              loadStyle(target.el as HTMLElement, value, prevProps[PROP_STYLE]);
